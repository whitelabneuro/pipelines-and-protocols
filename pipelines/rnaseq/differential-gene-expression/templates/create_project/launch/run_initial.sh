#!/bin/bash -l
#SBATCH --job-name=white_lab_dge_init
#SBATCH --output=/scratch/prj/bcn_whitema_rbp/[PROJECT_ID]/logs/%x-%j.out
#SBATCH --error=/scratch/prj/bcn_whitema_rbp/[PROJECT_ID]/logs/%x-%j.err
...
USER_SCRATCH_ROOT="/scratch/users/[KCL_USERNAME]"

QUANT_DIR="$(readlink -f /scratch/prj/bcn_whitema_rbp/[PROJECT_ID]/quantification)"
REFERENCE_GTF="$(readlink -f [PATH_TO_SCRATCH_LOCAL_GTF])"
...
PARAMS_FILE="${LAUNCH_DIR}/params.initial.yaml"
CREATE_CONFIG="${LAUNCH_DIR}/create.config"
METADATA_FILE="${INPUTS_DIR}/sample_metadata.csv"
CONTRASTS_FILE="${INPUTS_DIR}/contrasts.initial.csv"
...
RUN_MANIFEST="${RUN_METADATA_DIR}/run_manifest_initial.txt"
